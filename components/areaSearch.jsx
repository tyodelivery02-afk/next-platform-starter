  ...
  return (
    <div className="mb-2">
      <button
        ...
        <svg
          className="animate-spin w-10 h-10 mb-4 hover:opacity-70"
      ...
      {show && (
        <div className="fixed inset-0 bg-sky-100/10 backdrop-blur-sm flex items-center justify-center z-50 p-4 select-none">
          <div className="bg-sky-400 rounded-2xl shadow-2xl border border-sky-400 w-full max-w-5xl max-h-[85vh] overflow-hidden flex flex-col transition-all">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="bg-white rounded-xl border border-sky-500 overflow-hidden">
                <table className="w-full">
                  <thead>
                    <tr className="bg-sky-300 text-black">
                      <th className="border-b border-sky-500 p-4 text-left font-semibold w-1/5">
                        エリア
                      </th>
                      <th className="border-b border-sky-500 p-4 text-left font-semibold">
                        郵便番号先頭数字
                      </th>
                      <th className="border-b border-sky-500 p-4 text-center font-semibold w-24"></th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.map((region, i) => (
                      <tr
                        key={i}
                        className="hover:bg-yellow-200 transition-colors border-b border-sky-700/60 last:border-b-0"
                      >
                        <td className="p-4">
                          <input
                            className="bg-sky-200 border border-sky-500 rounded-lg w-full p-2.5 text-black placeholder-sky-200 focus:ring-2 focus:ring-yellow-400 focus:border-transparent transition-all"
                          ...
                          <div className="flex flex-wrap gap-2">
                            {region.keywords.map((kw, j) => (
                              <div key={j} className="flex items-center gap-1">
                                <input
                                  className="bg-sky-200 border border-sky-500 rounded-lg p-2 text-black placeholder-sky-200 focus:ring-2 focus:ring-yellow-400 focus:border-transparent transition-all max-w-[90px]"
                                  value={kw}
                                  onChange={(e) => updateValue(i, j, e.target.value)}
                                  placeholder="郵便番号先頭数字"
                                />
                                <button
                        ...
                        <td className="p-4 text-center">
                          <button
                            ...
                            className="text-yellow-600 hover:text-yellow-800 rounded-lg px-3 py-2 font-medium transition-all"
                            title="削除"
                          >
            ...
            {/* Footer */}
            <div className="flex items-center justify-between p-6 border-t border-sky-400 bg-sky-700">
              <button
                onClick={addRegion}
